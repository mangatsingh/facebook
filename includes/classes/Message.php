...
class Message {
	...
	public function getMessages($otherUser) {
		...
		while($row = mysqli_fetch_array($get_messages_query)) {
			...
			if($interval->y >= 1) {
				if($interval->y == 1)
					$chatingtime = $interval->y . " year ago"; //1 year ago
				else 
					$chatingtime = $interval->y . " years ago"; //1+ year ago
			}
				...
				if($interval->m == 1) {
					$chatingtime = $interval->m . " month". $days;
				}
				else {
					$chatingtime = $interval->m . " months". $days;
				}

			}
			else if($interval->d >= 1) {
				if($interval->d == 1) {
					$chatingtime = "Yesterday";
				}
				else {
					$chatingtime = $interval->d . " days ago";
				}
			}
			else if($interval->h >= 1) {
				if($interval->h == 1) {
					$chatingtime = $interval->h . " hour ago";
				}
				else {
					$chatingtime = $interval->h . " hours ago";
				}
			}
			else if($interval->i >= 1) {
				if($interval->i == 1) {
					$chatingtime = $interval->i . " minute ago";
				}
				else {
					$chatingtime = $interval->i . " minutes ago";
				}
			}
			else {
				if($interval->s < 30) {
					$chatingtime = "Just now";
				}
				else {
					$chatingtime = $interval->s . " seconds ago";
				}
			}
			if($row['viewed'] == 'yes') {
				$delivermsg = "<i class='fas fa-eye'></i>";
			}
			else {
				$delivermsg = "<i class='fas fa-eye-slash'></i>";
			}

			$div_top = ($user_to == $userLoggedIn) ? "<div class='message' id='green'>" : "<div class='message' id='blue'>" . $delivermsg;
			$button = "<span class='deleteButton' onclick='deleteMessage($id, this)'>X</span>";
			$timesofchat = "<span style='font-size: 12px;color: #4a4040;width:100%; display:inline-block; margin:-5px 5px -5px; text-align:right;'>$chatingtime</span>";
			$data = $data . $div_top . $button . $body . $timesofchat . "</div><br><br><br>";
		}
		return $data;
	}
